// 获取全局的根元素字体大小
export const GlobalFontSize = Number(getComputedStyle(document.documentElement).fontSize.replace('px', ''))
...
export const realSize = function (originSize: number) {
    return originSize / (GlobalFontSize / defaultFS)
}
...
export const getRealrem = function (px: number) {
    return Math.round((px / GlobalFontSize) * 10) / 10
}